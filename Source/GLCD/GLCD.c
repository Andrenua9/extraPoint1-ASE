...
void LCD_DrawDot(uint16_t xc, uint16_t yc, uint16_t radius, uint16_t color){
	while(radius>0){
	LCD_DrawCircle( xc,  yc, radius, color);
	radius--;
	}
	LCD_SetPoint(xc,yc, color);
	LCD_SetPoint(xc-1,yc-1, color);
	LCD_SetPoint(xc-1,yc+1, color);
	LCD_SetPoint(xc+1,yc+1, color);
	LCD_SetPoint(xc+1,yc-1, color);
}
void LCD_DrawCircle(uint16_t xc, uint16_t yc, uint16_t radius, uint16_t color) {
    int x = 0;
    int y = radius;
    int d = 3 - 2 * radius;
...
void LCD_DrawSquare(uint16_t xc, uint16_t yc, uint16_t color){
	int n = 13; //pixel del quadrato
	LCD_DrawLine(xc,yc,xc+n,yc,color);
	LCD_DrawLine(xc,yc,xc,yc+n,color);
	LCD_DrawLine(xc,yc+n,xc+n,yc+n,color);
	LCD_DrawLine(xc+n,yc,xc+n,yc+n,color);
	
}

void LCD_DrawSquareP(uint16_t xc, uint16_t yc, uint16_t color){
	int n = 13; //pixel del quadrato
	int i;
	for(i=0; i<12;i++)
		LCD_DrawLine(xc+i,yc,xc+i,yc+n-1,color);
}
